...
# Summarize total overnight stays by month and canton
monthly_stays <- overnight_stays %>%
  group_by(Month, Canton) %>%
  summarize(total_stays = sum(Stays, na.rm = TRUE)) %>%
  mutate(Month = factor(Month, levels = month.name))  # Ensure months are in calendar order (Jan, Feb, etc.)

# View the summarized data
head(monthly_stays)

# Plot seasonality by month for all cantons
# Plot seasonality by month, faceted by canton
ggplot(monthly_stays, aes(x = Month, y = total_stays)) +
  geom_line(aes(group = Canton, color = Canton)) +
  scale_y_continuous(labels = scales::comma) +  # Format y-axis with commas
  labs(title = "Seasonality of Overnight Stays by Month and Canton",
       x = "Month",
       y = "Total Overnight Stays") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotate x-axis labels for better readability
...
origin_summary_year <- combined_data %>%
  mutate(Origin = ifelse(CountryOfResidence == "Switzerland", "Switzerland", "Tourist")) %>%
...
ggplot(origin_summary_year, aes(x = as.factor(Year), y = percentage, fill = Origin)) +
  geom_bar(stat = "identity", position = "fill") +  # Stacked bar with percentage for each year
  scale_y_continuous(labels = scales::percent) +  # Show y-axis in percentage format
  labs(title = "Percentage of Overnight Stays by Swiss vs Tourist (2018-2023)",
       x = "Year",
       y = "Percentage") +
  theme_minimal()
